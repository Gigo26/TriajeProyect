...
class HospitalesFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        btnContactosImportantes.setOnClickListener {
            // Navega a Primeros Auxilios / Contactos
    ...
    private fun setupRecyclerView() {
        hospitalAdapter = HospitalAdapter(object : HospitalListener {
            override fun onHospitalClick(hospital: Hospital) {
                // Desplegar el diálogo de detalle (BottomSheetDialogFragment)
                val detailDialog = HospitalDetailDialogFragment.newInstance(hospital)
                detailDialog.show(childFragmentManager, "HospitalDetailDialog")
            }
        })

...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        userViewModel.usuario.observe(viewLifecycleOwner) { usuario ->
            usuario?.let {
                val firestoreService = com.moviles.triaje.network.FirestoreService()
                tvGreeting?.text = getString(R.string.hello_user, firestoreService.obtenerNombreFormateado(it))
            }
        }
